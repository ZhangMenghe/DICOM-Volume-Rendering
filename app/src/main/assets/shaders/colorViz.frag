...
vec3 transfer_scheme_hsv(float gray){
    if(uScheme == 1) return hsv2rgb(vec3(gray, 1.0, 1.0));
    else if(uScheme == 2) return hsv2rgb(vec3((1.0 -gray) * 180.0 / 255.0, 1.0, 1.0));
    return hex2rgb(u_hex_color_scheme[int(gray * 256.0)]);
}
...
vec4 get_mixture(float posx, vec3 gray){
    ...
    return vec4(transfer_scheme_hsv(posx), gray.r);
}
void main(){
    float intensity = vTexcoord.x;
    if(vTexcoord.y >0.66){
        if(uScheme == 0) fragColor = vec4(vec3(intensity), 1.0);
        else fragColor = vec4(transfer_scheme_hsv(vTexcoord.x), 1.0);
    }else{
        float gray = .0;
