...
//    vec4 final_color = Sample(storePos);
    uvec4 final_color = uvec4(imageLoad(srcTex, storePos).r);
...
//        imageStore(destTex_ray, storePos, ray_color);
        imageStore(destTex_ray,storePos,final_color);//vec4(final_color) / 256.0);
    #else
//        vec4 tex_color = UpdateTextureBased(final_color);
//        imageStore(destTex_tex, storePos, tex_color);
        imageStore(destTex_tex,storePos,vec4(final_color)/ 256.0);
