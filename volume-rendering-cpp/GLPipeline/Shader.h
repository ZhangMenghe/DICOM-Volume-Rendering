...
//	bool AddShaderFile(GLenum type, const char* filename);
	...
	//array
	static void Uniform(GLuint program, const GLchar* name, const int count, const int unit_size, const float* v){
		if(unit_size == 2)glUniform2fv(glGetUniformLocation(program, name), count, v);
		else if(unit_size==3)glUniform3fv(glGetUniformLocation(program, name), count, v);
	}
	//int array
	static void Uniform(GLuint program, const GLchar* name, const int count, const int* v){glUniform1iv(glGetUniformLocation(program, name), count, v);}
private:
