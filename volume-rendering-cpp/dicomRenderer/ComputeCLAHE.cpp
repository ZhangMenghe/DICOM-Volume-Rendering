...
GLuint LinkProgram(GLuint shaderID_1, GLuint shaderID_2=0) {
	...
	if (InfoLogLength > 0)
	{
		...
		LOGE("=====Failed to link shader %s", msg.c_str());
...
GLuint LoadComputeShader(std::string shaderCode){
	GLuint shaderID = 0;
	...
	if (InfoLogLength > 0)
	{
		...
		LOGE("=====Failed to load shader %s", msg.c_str());
		return 0;
	}
	if (shaderID == 0){
		return 0;
	...
	GLuint programID = LinkProgram(shaderID);
